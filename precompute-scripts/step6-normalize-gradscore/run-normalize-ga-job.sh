...
function run_normalize_ga_job {
    ...
    if [[ -n ${NORMALIZE_GA_TAG} ]]; then
        PROCESS_TAG_ARG="--processing-tag ${NORMALIZE_GA_TAG}"
    else
        PROCESS_TAG_ARG=""
    fi
    ...
    normalize_gradscore_cmd="${JAVA_EXEC} \
